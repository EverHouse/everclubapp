...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    changes: [
      "Improved: The staff booking calendar now loads significantly faster by removing a redundant external calendar lookup on every page load.",
    ]
  },
  {
    version: "8.98.40",
  ...
  },
  {
    ...
    changes: [
      "Removed: Session overlap checks removed from data integrity — the DB constraint prevents new overlaps, and Trackman bypasses are expected by design.",
      "New: 'Forever' option added to all issue exclusion durations so dismissed issues never come back.",
    ]
  },
  {
    version: "8.98.23",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fixed: Wallet pass downloads now work even when the auth header is not delivered by the network.",
    ]
  },
  {
    version: "8.97.95",
    ...
    changes: [
      ...
      "Fixed: Eventbrite event sync no longer stops entirely if a single event fails — other events continue syncing normally.",
      "Fixed: Event venue locations from Eventbrite now display correctly (previously could show 'Online Event' for in-person events).",
      "Improved: Better input validation across group and family billing management.",
    ]
  },
  {
    version: "8.97.93",
  ...
  },
  {
    ...
    title: "Stability & Reliability Improvements",
    changes: [
      "Fixed: Apple Wallet passes now update more reliably on member devices, resolving occasional \"Unauthorized\" errors when the Wallet app refreshes your membership card.",
      "Improved: App startup is now more resilient during high-traffic periods, reducing the chance of brief delays after deployments.",
      "Fixed: Minor memory cleanup improvement on the kiosk check-in screen for smoother long-running sessions.",
    ]
  },
  {
    version: "8.97.91",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fixed: App startup no longer floods the database with simultaneous requests — tasks now run in controlled batches",
    ]
  },
  {
    version: "8.97.46",
  ...
  },
  {
    ...
    changes: [
      "Fixed: Deleting cafe menu items no longer fails when the linked Stripe product doesn't exist",
    ]
  },
  {
    version: "8.97.6",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fixed: product.updated webhook now correctly parses marketing_features using parseMarketingFeatures — previously dumped encoded all_features entries as raw strings into the highlighted_features column",
  ...
  },
  {
    ...
    isMajor: true,
    changes: [
      "Fixed: All 241 icons across the entire app are now inline SVGs instead of a font — icons render instantly without any external loading, network requests, or font detection delays",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fixed: Cross-system drift and HubSpot member sync checks now return stable, deterministic results — both were using random sampling so results could differ between runs",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fixed: Stripe reconnect now creates a new Stripe customer when none exists instead of failing — ensures all members get linked to a Stripe customer ID",
      "Improved: Bulk reconnect now shows a helpful warning (not error) when partial failures occur",
    ]
  },
  {
    version: "8.87.88",
  ...
  },
  {
    ...
    changes: [
      "Fixed: Booking passes now void immediately when a cancellation is pending (awaiting Trackman cleanup), and self-heal if a retry is attempted",
      "Fixed: Membership passes now update when your subscription is cancelled, past due, suspended, or terminated after grace period",
      "Fixed: Family/group sub-members also get their passes updated when the primary account's subscription status changes",
      "Fixed: Membership passes now update when a subscription is resumed (reactivated) — including all sub-members",
      "Fixed: Membership passes update when your tier is changed by staff or through subscription upgrades/downgrades",
      "Fixed: Passes update when a new member activates via checkout link or staff invite",
      "Fixed: Passes update when members are added to family or corporate billing groups",
      "Fixed: Changing your name on the profile page now updates your Apple Wallet membership pass",
      "Fixed: Using or getting refunded a guest pass now immediately updates the remaining count on your membership pass",
      "Fixed: Monthly guest pass reset and Stripe subscription reconciliation now also trigger pass updates",
      "Fixed: Archiving a member now updates their wallet pass to reflect the archived status",
  ...
  },
  {
    ...
    changes: [
      "Removed ~48 unused React Query hooks created during the data-fetching migration that were never adopted by components — covers useAdminQueries (26 hooks including useStaffActivity), useMemberPageQueries (16 hooks), useBookingsQueries (12 hooks), plus 1 each from financials and cafe query files",
  ...
  },
  {
    ...
    changes: [
      "New: Members with a saved card can now pay with a single tap — no need to fill out the payment form",
      "Works for both booking prepayments and invoice payments from the History page",
      ...
      "Full audit trail for all saved card member payments, just like staff-initiated charges",
    ]
  },
  {
    version: "8.87.8",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fix: Google sign-in button no longer re-initializes the SDK on every parent re-render — uses stable callback refs to prevent the SDK from overwriting active auth popups mid-flow, which could lose credentials",
      "Fix: Profile page link/unlink now awaits the status refetch before showing the success toast — prevents the button from briefly flashing the old state",
      "Fix: All link/unlink callbacks on the profile page and login page are memoized to prevent unnecessary re-renders of sign-in buttons",
    ]
  },
  {
    version: "8.86.4",
  ...
  },
  {
    ...
    changes: [
      ...
      "Fix: Billing calculation errors during check-in resolved — usage calculator and approval service now handle edge cases that previously caused incorrect fee amounts",
      "Fix: Staff manual booking modal now validates all required fields before submission — prevents incomplete bookings from being created",
      ...
      "Improvement: Data integrity actions now validate inputs more strictly — prevents errors when running integrity fixes with invalid parameters",
  ...
  },
  {
    ...
    changes: [
      ...
      "Feature: Billing and payment now use a unified modal for invoices — cleaner experience when viewing and paying booking fees",
      ...
      "Fix: Fee calculation now excludes cancelled bookings from daily usage — cancelled sessions no longer eat into a member's daily time allowance",
      ...
      "Fix: Billing history date formatting corrected — dates now display consistently across all entries",
      ...
      "Fix: Stale invoices are cleaned up before creating new payment intents — prevents payment errors from leftover invoice data",
      "Fix: Booking IDs are now correctly extracted from invoice metadata — fixes billing history lookups for older invoices",
      ...
      "Fix: 'View' button on payment receipts only appears for successfully completed payments — no more broken links on failed attempts",
  ...
  },
  {
    ...
    changes: [
      ...
      "Improvement: Financial page loading skeletons improved on mobile — smoother experience while payment data loads",
      ...
      "Fix: Booking sessions are now properly linked to their bookings at creation — prevents orphaned sessions",
      "Fix: Booking session participants are updated when the roster changes — old participants removed and new ones added automatically on startup",
      "Fix: Booking session creation and overlap detection logic improved — prevents errors when creating sessions for adjacent time slots",
      "Fix: Fee backfill process now computes fees for newly created sessions — ensures historical bookings have correct billing data",
      "Fix: Booking request format interpretation improved — handles various date and time formats more reliably",
      "Fix: Lesson events from Google Calendar no longer create facility closure notices — these are filtered out during event sync",
      "Fix: Participant transaction errors resolved — roster changes no longer cause database constraint violations",
      "Fix: HubSpot queue processing no longer stalls on invalid entries — skips malformed jobs instead of blocking the queue",
      "Fix: Session creation error handling improved — prevents unnecessary error logs when sessions already exist",
      "Fix: App error boundaries simplified — cleaner error recovery without excessive state resets",
  ...
  },
  {
    ...
    changes: [
      ...
      "Security: IP spoofing and session bypass vulnerabilities addressed — audit log and session manager hardened against forged requests",
      "Security: Rate limiting improved to prevent duplicate OTP emails and IP spoofing — authentication endpoints now enforce stricter request validation",
      "Fix: Members can no longer accidentally link their account to themselves — self-linking prevention added to the user linking system",
      "Fix: Member directory now shows correct results for all search queries — visibility and data consistency improvements",
      "Fix: User roles are now automatically corrected when staff status changes — prevents staff members from losing admin access after account updates",
      "Fix: Trackman booking imports now match members using placeholder emails — reduces the number of unmatched bookings that need manual assignment",
      "Fix: Unmatched bookings during CSV imports are now handled more gracefully — broader email matching and better cancellation handling",
      "Fix: All bookings are now properly marked as matched after assignment — prevents bookings from lingering in the unmatched queue",
      "Fix: HubSpot sync now correctly maps membership status and billing provider — prevents incorrect CRM data for members who switched billing",
      "Fix: HubSpot data integrity sync now fetches contact IDs directly from the database — more reliable sync operations",
      "Improvement: Bulk 'Mark All Waivers Signed' action added to the data integrity dashboard — quickly resolve waiver flags for large groups",
      "Improvement: Activity log section removed from data integrity page — cleaner interface focused on actionable checks",
