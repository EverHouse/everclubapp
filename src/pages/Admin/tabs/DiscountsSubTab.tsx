...
const DiscountsSubTab: React.FC<DiscountsSubTabProps> = ({ onCreateClick }) => {
  ...
  if (error && coupons.length === 0) {
    return (
      <div className="flex flex-col items-center text-center py-12 px-6 rounded-xl border-2 border-dashed border-red-200 dark:border-red-500/25 bg-red-50 dark:bg-red-900/10">
      ...
      {coupons.length === 0 ? (
        ...
        <div ref={discountsRef} className="space-y-3">
          {coupons.map((coupon, index) => (
            <div 
              key={coupon.id} 
              className={`p-4 rounded-xl border transition-colors tactile-row ${
                coupon.valid 
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-wider w-fit px-2 py-0.5 rounded-[4px] ${
                      coupon.percentOff 
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider w-fit bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-[4px]">
                      {getDurationLabel(coupon)}
                    </span>
                    {!coupon.valid && (
                      <span className="text-[10px] font-bold uppercase tracking-wider w-fit bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-[4px]">
                        Inactive
                      </span>
                    )}
                  </div>
