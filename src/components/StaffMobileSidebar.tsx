...
import { springPresets, sidebarVariants, backdropVariants, menuContainerVariants, menuItemVariants } from '../utils/motion';
...
export const StaffMobileSidebar: React.FC<StaffMobileSidebarProps> = ({
  ...
  const NavButton: React.FC<{ item: typeof MAIN_NAV_ITEMS[number] }> = ({ item }) => {
    ...
    return (
      <motion.div variants={prefersReduced ? undefined : menuItemVariants}>
      <button
        ...
        <div className="fixed inset-0" style={{ zIndex: 'var(--z-modal, 100)' }}>
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-4" data-scroll-lock-allow>
              <motion.div className="space-y-0.5" variants={prefersReduced ? undefined : menuContainerVariants} initial={prefersReduced ? false : 'hidden'} animate="visible">
