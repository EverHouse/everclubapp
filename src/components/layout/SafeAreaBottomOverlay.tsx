...
export const SafeAreaBottomOverlay: React.FC<SafeAreaBottomOverlayProps> = ({ children }) => {
  ...
  const overlayContent = (
    <div 
      className={`fixed inset-x-0 bottom-0 pointer-events-none transition-all duration-normal ease-out lg:hidden ${isHidden ? 'translate-y-[calc(100%+env(safe-area-inset-bottom,0px))] opacity-0' : 'translate-y-0 opacity-100'}`}
