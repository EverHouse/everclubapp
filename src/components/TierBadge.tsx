...
const TierBadge: React.FC<TierBadgeProps> = ({ tier, size = 'sm', showNoTier = false, lastTier, membershipStatus }) => {
  ...
  if (isInactive && tier && tier.trim() !== '') {
    const displayLastTier = getDisplayTier(tier);
    const statusLabel = membershipStatus.charAt(0).toUpperCase() + membershipStatus.slice(1).toLowerCase();
