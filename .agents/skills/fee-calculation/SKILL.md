...
11. **Already-paid guard (v8.68.0, fixed v8.98.22).** Fee recalculation skips participants with `payment_status = 'paid'`. When `cached_fee_cents = 0` (cleared after payment) and computed fee > $0, the system only resets to `pending` if there is NO real Stripe payment intent (`stripe_payment_intent_id` starting with `pi_`). Participants with a real Stripe PI are preserved as paid — their $0 `cached_fee_cents` correctly means "balance settled," not "no payment made."
