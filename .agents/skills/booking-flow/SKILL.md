...
3b. **Conflict detection covers 4 sources (v8.87.87).** Both `checkResourceOverlap()` (member path in `bookingCreationGuard.ts`) and `checkBookingConflict()` (shared path in `bookingValidation.ts`) check: (1) `booking_requests` with 6 active statuses, (2) `trackman_bay_slots` where `status='booked'`, (3) `trackman_unmatched_bookings` where `resolved_at IS NULL` and no linked booking_request exists, and (4) `booking_sessions` with an active booking. All 4 paths use fail-closed error handling (throw 503 on query failure) with table-not-found tolerance (PostgreSQL error `42P01` → skip gracefully). Staff paths (`staffActions.ts` via `checkAllConflicts`, `staff-conference-booking.ts` via `checkBookingConflict`) inherit the same 4-source checks. Frontend `useBookGolf.ts` does a pre-submission availability re-fetch to catch race conditions client-side.
...
8. **Conflict check must include all 6 active statuses.** `pending`, `pending_approval`, `approved`, `confirmed`, `attended`, `cancellation_pending`. For `booking_sessions` subquery, use `NOT IN ('cancelled', 'deleted', 'declined')` which is equivalent plus includes terminal `expired`/`no_show` (harmless — those are past bookings).
