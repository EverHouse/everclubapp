...
3a. **Advisory locks on booking creation.** `acquireBookingLocks()` in `bookingCreationGuard.ts` acquires `pg_advisory_xact_lock` on resource and member using a **strict hierarchical order** — resource lock ALWAYS acquired first, then user lock (v8.88.7). Lock keys use fixed domain integers as the first parameter (1 for users, 2 for resources) to guarantee domain isolation and prevent hash collisions (v8.97.41). Email is normalized to `trim().toLowerCase()` before hashing to match the `LOWER()` comparison in the pending-check query — without this, differently-cased emails would acquire separate locks and bypass the rate limiter. `checkResourceOverlap()` uses `SELECT FOR UPDATE` to verify no overlapping bookings exist (multi-row — must use `ORDER BY id ASC` per convention 18a in `project-architecture`). Roster edits use `roster_version` optimistic locking with `SELECT FOR UPDATE` (v8.86.0, single-row by booking ID).
3b. **Conflict detection covers 4 sources (v8.87.87, hardened v8.88.6).** Both `checkResourceOverlap()` (member path in `bookingCreationGuard.ts`) and `checkBookingConflict()` (shared path in `bookingValidation.ts`) check: (1) `booking_requests` with 7 occupied statuses (see hard rule 8), (2) `trackman_bay_slots` where `status='booked'`, (3) `trackman_unmatched_bookings` where `resolved_at IS NULL` and no linked booking_request exists, and (4) `booking_sessions` with an active booking. All 4 paths use fail-closed error handling (throw 503 on query failure) with table-not-found tolerance (PostgreSQL error code `42P01` only — checked via `getErrorCode()` to handle wrapped errors). **CRITICAL**: `checkResourceOverlap()` runs inside a `db.transaction()`, so optional-table queries (trackman, sessions) MUST use `SAVEPOINT` / `ROLLBACK TO SAVEPOINT` to avoid poisoning the transaction if 42P01 fires — a plain try/catch swallows the JS error but PostgreSQL marks the transaction as aborted and all subsequent queries fail. `checkBookingConflict()` uses `db.execute()` (no transaction), so SAVEPOINTs are not needed there. Staff paths (`staffActions.ts` via `checkAllConflicts`, `staff-conference-booking.ts` via `checkBookingConflict`) inherit the same 4-source checks. Frontend `useBookGolf.ts` does a pre-submission availability re-fetch to catch race conditions client-side. The pending-request rate limiter in `acquireBookingLocks()` checks `status IN ('pending', 'pending_approval')` to prevent users from submitting new requests while an existing one is still being processed.
