...
7. **Roster lock after paid invoice.** `enforceRosterLock()` blocks edits after invoice is paid. Staff can override with reason. `isBookingInvoicePaid()` checks Stripe first; on Stripe failure, falls back to local `booking_participants.payment_status` — if any participant with fees is marked `paid`, the lock is enforced. This prevents fail-open behavior when Stripe is unreachable.
