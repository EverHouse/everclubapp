...
14. **Dispute handlers cover ALL payment types (v8.97.42).** `handleChargeDisputeCreated`, `handleChargeDisputeClosed`, and `handleChargeDisputeUpdated` check `terminal_payments` first, then fall back to `stripe_payment_intents` for web/booking fee payments. If neither table matches, staff receives an urgent notification for manual investigation. When `paymentIntentId` is null (legacy charges without a payment_intent), the handler logs a warning and notifies staff with the charge ID. Dispute-won reactivation is guarded: `handleChargeDisputeClosed` checks for OTHER open disputes across BOTH tables on the same member AND verifies the Stripe subscription status. Reactivation is blocked if subscription is `past_due`, `unpaid`, or `canceled`. Subscription lookup failures are fail-closed (block reactivation, alert staff). The `membershipAction`/`webMembershipAction` variable tracks whether reactivation actually happened so audit logs and notifications reflect the real outcome.
