...
All webhooks check `event.id` via `tryClaimEvent()`. Before creating ANY PaymentIntent for a booking, query `stripe_payment_intents` for an existing open intent first. Include `metadata.bookingId` on all intents.
