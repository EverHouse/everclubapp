...
9. **Auto-heal architecture (v8.98.21).** Integrity checks are split into two groups:
   - **Nightly checks (always run)**: 27 checks including all external system syncs + 4 promoted auto-heal checks. When `triggeredBy='scheduled'`, `autoFix=true` is derived automatically. Auto-fix-capable checks: Stripe subscription sync, tier reconciliation, HubSpot sync mismatch, bookings without sessions (auto-creates sessions), orphaned payment intents (auto-cancels), billing provider hybrid state (auto-backfills subscription IDs), stuck transitional members (auto-cleans dead subscriptions), approved bookings for inactive members (auto-cancels), usage ledger gaps, wellness block gaps, stuck push notifications, wallet pass booking sync (auto-voids for terminal bookings only — reactivation is manual).
   - **Manual-only checks (legacy)**: 15 checks gated behind `includeLegacy=true` (default for manual runs). These are backstop diagnostics for issues already prevented by DB constraints (guest pass drift, duplicate Stripe customers, etc.).
   - **Removed checks**: `checkSessionOverlaps` and `checkOverlappingBookings` — both eliminated (v8.98.24). The DB exclusion constraint prevents app-created overlaps; Trackman deliberately bypasses it via `SET LOCAL app.bypass_overlap_check = 'true'` because Trackman is the source of truth for physical bay usage. All ~131 legacy overlap issues were Trackman-sourced false positives.
   - **Detection-only (no auto-fix)**: Fee snapshot Stripe drift — amount divergences require human review due to refund/accounting edge cases. Wallet pass voided-but-active-booking — requires manual review.
   - Auto-fix results include `autoFixedCount`/`autoFixSummary` in `IntegrityCheckResult`. Dashboard shows green "Auto-resolved" badges and summary banners for auto-fixed items. Scheduler logs total auto-fix counts and summaries.
...
12. **DB-level guards for stale/unpaid bookings.** `trg_guard_stale_pending` rejects approving bookings >2hr past start time. `trg_guard_attended_unpaid` blocks transition to attended if unpaid fee snapshots exist. `trg_guard_approve_inactive` blocks approving/confirming bookings when member status is inactive/suspended/cancelled/terminated/archived. `trg_guard_checkin_no_session` blocks check-in/attended transition when no billing session is linked. All use `app.bypass_status_check` escape hatch.
...
| **High** | Notify when count > threshold (10) | Tier Reconciliation, Billing Provider Hybrid State, Lingering Payment Intents on Terminal Bookings, Usage Ledger Gaps (auto-fix), Sessions Exceeding Resource Capacity, Fee Snapshot Stripe Drift, Wellness Block Coverage, Stuck Unpaid Bookings, Unresolved Failed Side Effects |
...
| **Low** | Informational | Sessions Without Participants, Stale Past Tours, Stale Push Subscriptions |
