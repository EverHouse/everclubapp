...
9. **Auto-heal architecture (v8.98.21).** Integrity checks are split into two groups:
   - **Nightly checks (always run)**: 28 checks including all external system syncs + 4 promoted auto-heal checks. When `triggeredBy='scheduled'`, `autoFix=true` is derived automatically. Auto-fix-capable checks: Stripe subscription sync, tier reconciliation, HubSpot sync mismatch, bookings without sessions (auto-creates sessions), orphaned payment intents (auto-cancels), billing provider hybrid state (auto-backfills subscription IDs), stuck transitional members (auto-cleans dead subscriptions), approved bookings for inactive members (auto-cancels), usage ledger gaps, session overlaps (auto-ack past), wellness block gaps, stuck push notifications, wallet pass booking sync (auto-voids for terminal bookings only — reactivation is manual).
   - **Manual-only checks (legacy)**: 16 checks gated behind `includeLegacy=true` (default for manual runs). These are backstop diagnostics for issues already prevented by DB constraints (overlapping bookings, guest pass drift, duplicate Stripe customers, etc.).
