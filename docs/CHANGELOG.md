...
- **Fixed**: `TabTransition` component rewritten to use enter-only animation. The previous exit→enter approach had two bugs: (1) without freezing, it faded out the new content then faded it back in; (2) with freezing, it held stale children (e.g. player count selector) visible while content outside the transition (e.g. heading) had already updated, creating a visual mismatch. Now simply applies `animate-tab-enter` fade-in when `activeKey` changes — old content disappears instantly, new content fades in over 200ms. Affects all pages using `TabTransition`: BookGolf, Wellness, History, AdminDashboard, FinancialsTab, BlocksTab.
