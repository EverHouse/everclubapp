...
- **Note**: `/api/bookings/:id/participants/preview-fees` is also returning 500 in production — the underlying Postgres error is truncated in logs but this is likely a missing column from a migration that hasn't run yet. A redeploy should fix it along with the merch table issue.
- **Files changed**: `server/routes/merch.ts`, `server/routes/cafe.ts`, `server/routes/calendar.ts`, `server/routes/tours.ts`, `server/routes/waivers.ts`, `server/routes/hubspot/forms.ts`, `server/routes/hubspot/contacts.ts`, `server/routes/hubspot/shared.ts`, `server/routes/closures/crud.ts`, `server/routes/closures/admin.ts`, `server/routes/members/dashboard.ts`, `server/routes/members/admin-actions.ts`, `server/routes/members/visitors.ts`, `server/routes/members/communications.ts`, `server/routes/tierFeatures.ts`, `server/core/memberSyncRelevant.ts`, `server/core/memberSyncOperations.ts`, `server/core/memberSyncCommLogs.ts`, `server/core/integrity/externalSystemChecks.ts`, `server/core/integrity/stripeChecks.ts`, `server/core/integrity/hubspotChecks.ts`, `server/core/integrity/memberChecks.ts`, `server/core/integrity/core.ts`, `server/core/hubspot/request.ts`, `server/core/hubspot/members.ts`, `server/core/hubspot/companies.ts`, `server/routes/dataTools/stripe-tools.ts`, `server/routes/dataTools/maintenance.ts`
...
- **Stale Stripe price cleanup**: Added `archiveStalePrices()` helper in `productCreation.ts` that runs after each fee product is ready (overage, guest pass, day pass coworking, day pass golf sim). Lists all active prices for the product and archives any that don't match the current active price ID. Prevents accumulation of orphaned prices on Stripe products.
- **Files changed**: `server/routes/stripe/terminal.ts`, `server/core/stripe/webhooks/handlers/payments.ts`, `server/core/stripe/productCreation.ts`
...
- **Standardized**: Replaced all manual `error instanceof Error ? error.message : String(error)` patterns with `getErrorMessage()` across 19 server files, ensuring consistent Drizzle cause-unwrapping everywhere: `walletPass/passGenerator.ts`, `core/databaseCleanup.ts`, `core/sessionCleanup.ts`, `core/errorAlerts.ts`, `core/dataAlerts.ts`, `core/trackman/trackmanImport.ts`, `core/trackman/trackmanRescan.ts`, `core/memberSyncRelevant.ts`, `core/memberSyncOperations.ts`, `core/bookingService/approvalTypes.ts`, `core/visitors/autoMatchService.ts`, `core/mindbody/import.ts`, `routes/dataIntegrity/sync.ts`, `routes/guestPasses.ts`, `routes/roster.ts`, `routes/calendar.ts`, `routes/closures/helpers.ts`, `routes/stripe/subscriptions.ts`, `routes/stripe/member-payments/saved-cards.ts`, `routes/trackman/webhook-reprocess.ts`, `routes/trackman/webhook-billing.ts`, `routes/trackman/webhook-matching.ts`
...
- **Fixed**: `handleProductUpdated` webhook handler in `catalog.ts` now uses `parseMarketingFeatures` to correctly decode marketing_features into separate `highlighted_features` and `all_features` columns. Previously, it treated all entries (including encoded `⌁af2:` entries) as literal highlighted feature strings, corrupting the `highlighted_features` column. The deferred `pullTierFeaturesFromStripe` call eventually corrected this, but the initial write was wrong and briefly visible.
