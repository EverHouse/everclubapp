...
export async function syncMemberTierFromStripe(
  ...
  try {
    ...
    try {
      ...
      logger.error('[TierSync] HubSpot sync failed:', { extra: { error: getErrorMessage(hubspotError) } });
    }
    
    return { success: true, newTier: tierSlug, newTierId: tierId };
  } catch (error: unknown) {
    logger.error('[TierSync] Error syncing tier:', { extra: { error: getErrorMessage(error) } });
...
export async function syncMemberStatusFromStripe(
  ...
  try {
    ...
    logger.error('[TierSync] Error syncing status:', { extra: { error: getErrorMessage(error) } });
    return { success: false, error: getErrorMessage(error) };
  }
}
...
export async function getTierFromPriceId(stripePriceId: string): Promise<{
  ...
  try {
    ...
    logger.error('[TierSync] Error getting tier from price ID:', { extra: { error: getErrorMessage(error) } });
    return null;
  }
}
...
export async function validateTierConsistency(email: string): Promise<{
  ...
  try {
    ...
    logger.error('[TierSync] Error validating tier consistency:', { extra: { error: getErrorMessage(error) } });
