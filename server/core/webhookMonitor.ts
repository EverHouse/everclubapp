...
export async function getWebhookEvents(params: WebhookQueryParams): Promise<{ events: WebhookEvent[]; total: number }> {
  ...
  const events = (result.rows as unknown as WebhookEventRow[]).map((row: WebhookEventRow) => ({
    ...
    processedAt: row.processed_at instanceof Date ? row.processed_at.toISOString() : (row.processed_at ? new Date(String(row.processed_at) + 'Z').toISOString() : null),
    ...
    createdAt: row.created_at instanceof Date ? row.created_at.toISOString() : new Date(String(row.created_at) + 'Z').toISOString(),
    retryCount: Number(row.retry_count || 0),
    lastRetryAt: row.last_retry_at instanceof Date ? row.last_retry_at.toISOString() : (row.last_retry_at ? new Date(String(row.last_retry_at) + 'Z').toISOString() : null),
