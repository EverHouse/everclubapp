...
export async function previewMerge(primaryUserId: string, secondaryUserId: string): Promise<MergePreview> {
  ...
  const extractCount = (result: { rows: Array<Record<string, unknown>> }): number =>
    parseInt((result.rows as Array<Record<string, unknown>>)[0]?.count as string || '0', 10);

  const [
    bookingsCount, visitsCount, wellnessCount, eventRsvpsCount,
    notificationsCount, memberNotesCount, guestCheckInsCount, usageLedgerCount,
    bookingParticipantsCount, dayPassCount, legacyPurchasesCount, groupMembersCount,
    pushSubscriptionsCount, dismissedNoticesCount, billingGroupsCount, bugReportsCount,
    dataExportCount, hubspotDealsCount, stripePaymentIntentsCount,
  ] = await Promise.all([
    db.execute(sql`SELECT COUNT(*) as count FROM booking_requests WHERE LOWER(user_email) = ${secondaryEmail} OR user_id = ${secondaryUserId}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM booking_requests WHERE (LOWER(user_email) = ${secondaryEmail} OR user_id = ${secondaryUserId}) AND status = 'attended'`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM wellness_enrollments WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM event_rsvps WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM notifications WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM member_notes WHERE LOWER(member_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM guest_check_ins WHERE LOWER(member_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM usage_ledger WHERE member_id = ${secondaryUserId}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM booking_participants WHERE user_id = ${secondaryUserId}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM day_pass_purchases WHERE user_id = ${secondaryUserId} OR LOWER(purchaser_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM legacy_purchases WHERE user_id = ${secondaryUserId} OR LOWER(member_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM group_members WHERE LOWER(member_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM push_subscriptions WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM user_dismissed_notices WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM billing_groups WHERE LOWER(primary_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM bug_reports WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM data_export_requests WHERE LOWER(user_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM hubspot_deals WHERE LOWER(member_email) = ${secondaryEmail}`).then(extractCount),
    db.execute(sql`SELECT COUNT(*) as count FROM stripe_payment_intents WHERE user_id = ${secondaryUserId}`).then(extractCount),
  ]);
...
async function executeMergeInternal(
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`UPDATE users SET
         ...
         membership_status_changed_at = CASE WHEN membership_status IS DISTINCT FROM 'merged' THEN NOW() ELSE membership_status_changed_at END,
         email = ${archivedEmail},
