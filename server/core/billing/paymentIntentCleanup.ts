...
export async function cancelPendingPaymentIntentsForBooking(bookingId: number, options?: { skipSnapshotUpdate?: boolean }): Promise<void> {
  try {
    ...
    if (!options?.skipSnapshotUpdate) {
      ...
      const orphanCount = (orphanResult.rows as unknown[]).length;
      if (orphanCount > 0) {
        logger.info(`[Payment Intent Cleanup] Cancelled ${orphanCount} orphaned fee snapshots (no PI) for booking ${bookingId}`);
      }
    }
  } catch (e: unknown) {
    logger.warn('[Payment Intent Cleanup] Non-critical cleanup failed:', { extra: { error: getErrorMessage(e) } });
  }
}
...
export async function refundSucceededPaymentIntentsForBooking(bookingId: number): Promise<number> {
  ...
  try {
    ...
    logger.warn('[PI Cleanup] Non-critical refund cleanup failed:', { extra: { error: getErrorMessage(e) } });
  }
  return refundCount;
}
