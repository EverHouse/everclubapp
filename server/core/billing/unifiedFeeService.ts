...
export async function computeFeeBreakdown(params: FeeComputeParams): Promise<FeeBreakdown> {
  ...
  if (params.sessionId || params.bookingId) {
    ...
    if (!sessionData.bookingId) {
      logger.warn('[FeeBreakdown] Session has no bookingId attached', { extra: { sessionId: sessionData.sessionId } });
    } else {
  ...
  const nonMemberMinutes = emptySlotMinutes;
  ...
  if (!options?.skipCascade) {
    try {
      const sessionInfo = await db.execute(sql`
        SELECT bs.id, bs.session_date, br.user_email, br.start_time
        FROM booking_sessions bs
        JOIN booking_requests br ON br.session_id = bs.id
        WHERE bs.id = ${sessionId}
        LIMIT 1
