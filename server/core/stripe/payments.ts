...
export async function confirmPaymentSuccess(
  ...
  try {
    ...
    const localRecord = txClient
      ? await txClient.query(
          `SELECT id, user_id, stripe_payment_intent_id, amount_cents, purpose, booking_id, session_id, product_name, status FROM stripe_payment_intents WHERE stripe_payment_intent_id = $1`,
          [paymentIntentId]
        ) as { rows: Record<string, unknown>[] }
      : await db.execute(sql`SELECT id, user_id, stripe_payment_intent_id, amount_cents, purpose, booking_id, session_id, product_name, status FROM stripe_payment_intents WHERE stripe_payment_intent_id = ${paymentIntentId}`);
