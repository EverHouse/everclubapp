...
export async function reconcileDailyPayments() {
  ...
  try {
    ...
    logger.error('[Reconcile] Error during reconciliation:', { extra: { error: getErrorMessage(error) } });
    throw error;
  }
}
...
export async function reconcileSubscriptions() {
  ...
  try {
      ...
      for (const subscription of subscriptions.data) {
        ...
        try {
          ...
          try {
            ...
            logger.error(`[Reconcile] HubSpot sync failed for ${customerEmail}:`, { extra: { error: getErrorMessage(hubspotError) } });
          }
    ...
    logger.error('[Reconcile] Error during subscription reconciliation:', { extra: { error: getErrorMessage(error) } });
    throw error;
  }
}
          ...
          if (participantResult.rowCount && participantResult.rowCount > 0) {
            ...
            if (userIds.length > 0) {
              ...
              const emailResult = userIdInts.length > 0
                ? await db.execute(sql`SELECT email FROM users WHERE id = ANY(${sql`ARRAY[${sql.join(userIdInts.map(id => sql`${id}`), sql`, `)}]::int[]`})`)
                : { rows: [] };
