  ...
  try {
    ...
    if (primaryStripeSubscriptionId) {
      ...
      try {
        ...
        logger.error('[GroupBilling] Failed to update Stripe on member removal:', { extra: { error: getErrorMessage(stripeErr) } });
        if (newStripeItemId) {
          try {
            ...
            logger.error(`[GroupBilling] CRITICAL: Failed to delete newly created Stripe subscription item ${newStripeItemId}. Customer may be double-billed. Manual intervention required.`, { extra: { error: getErrorMessage(rollbackErr) } });
          }
        }
        try {
          await db.execute(
          ...
          logger.error(`[GroupBilling] CRITICAL: Failed to compensate DB after Stripe failure. Manual intervention required.`, { extra: { error: getErrorMessage(compensateErr) } });
        }
    ...
    logger.error('[GroupBilling] Error removing corporate member:', { extra: { error: getErrorMessage(err) } });
  ...
  try {
    ...
    if (stripeSubscriptionItemId) {
      try {
        ...
        logger.error('[GroupBilling] Failed to remove Stripe subscription item:', { extra: { error: getErrorMessage(stripeErr) } });
        try {
          await db.execute(
    ...
    logger.error('[GroupBilling] Error removing group member:', { extra: { error: getErrorMessage(err) } });
    return { success: false, error: 'Failed to remove member. Please try again.' };
  }
}
...
export async function handleSubscriptionItemsChanged(
  ...
  try {
    ...
    logger.error('[GroupBilling] Error handling subscription items change:', { extra: { error: getErrorMessage(err) } });
  }
}
  ...
  try {
    ...
    if (emailsToDeactivate.length > 0) {
      try {
        ...
        logger.error('[GroupBilling] HubSpot sync failed for cancelled sub-members:', { extra: { error: getErrorMessage(hubspotErr) } });
      }
    }
    ...
    logger.error('[GroupBilling] Error handling primary subscription cancellation:', { extra: { error: getErrorMessage(err) } });
    throw err;
  }
}
