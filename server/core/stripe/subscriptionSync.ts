...
export async function syncActiveSubscriptionsFromStripe(): Promise<SubscriptionSyncResult> {
  ...
  try {
          ...
          try {
            for (const status of ['active', 'trialing', 'past_due'] as const) {
              ...
              for (const sub of custSubs.data) {
                (sub as { customer: string | Stripe.Customer }).customer = cust;
    ...
    for (let i = 0; i < subscriptions.length; i += batchSize) {
      ...
      for (const subscription of batch) {
        try {
          ...
          const existingUser = await db.execute(sql`SELECT u.id, mt.name as tier, u.stripe_customer_id, u.stripe_subscription_id, u.hubspot_id, u.first_name, u.last_name, u.updated_at FROM users u LEFT JOIN membership_tiers mt ON u.tier_id = mt.id WHERE LOWER(u.email) = ${email}`);
          ...
          logger.error(`[Stripe Sync] Error processing subscription:`, { extra: { error: getErrorMessage(err) } });
        }
      }
    ...
    logger.error('[Stripe Sync] Fatal error during sync:', { extra: { error: getErrorMessage(error) } });
