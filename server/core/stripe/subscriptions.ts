  ...
  try {
    ...
    const subscriptions = await stripe.subscriptions.list({
      ...
      expand: [
        ...
        'data.schedule',
        'data.pending_update.subscription_items.price.product'
      ],
    });
    ...
    return {
      ...
      subscriptions: subscriptions.data.map(sub => {
        ...
        if (sub.pending_update?.subscription_items && sub.pending_update.subscription_items.data.length > 0) {
          ...
          const pendingProduct = pendingPrice?.product as Stripe.Product;
          const effectiveTimestamp = sub.pending_update.billing_cycle_anchor
            || sub.pending_update.expires_at
            || sub.current_period_end;

          if (pendingPrice && effectiveTimestamp) {
            pendingUpdate = {
              newPriceId: pendingPrice.id,
