  ...
  try {
    ...
    for (const rule of rules) {
      try {
        ...
        logger.error(`[Discount Sync] Error syncing ${rule.discountTag}:`, { extra: { error: getErrorMessage(error) } });
        results.push({
          discountTag: rule.discountTag,
          discountPercent: rule.discountPercent,
          success: false,
    ...
    logger.error('[Discount Sync] Fatal error:', { extra: { error: getErrorMessage(error) } });
  ...
  try {
    ...
    for (const rule of rules) {
      ...
      try {
        ...
        logger.debug('[Discount Sync] Coupon not found in Stripe', { extra: { couponId, error: getErrorMessage(err) } });
    ...
    logger.error('[Discount Sync] Error getting status:', { extra: { error: getErrorMessage(error) } });
    return [];
  }
}
...
export async function findOrCreateCoupon(discountTag: string, discountPercent: number): Promise<string | null> {
  ...
  try {
    ...
    logger.error(`[Discount] Error finding/creating coupon for ${discountTag}:`, { extra: { error: getErrorMessage(error) } });
    return null;
  }
}
