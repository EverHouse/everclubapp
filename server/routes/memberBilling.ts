...
router.get('/api/member-billing/:email/outstanding', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error fetching outstanding balance', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.put('/api/member-billing/:email/source', isStaffOrAdmin, validateBody(billingSourceSchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error updating billing source', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/pause', isStaffOrAdmin, validateBody(billingPauseSchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error pausing subscription', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/resume', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error resuming subscription', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/cancel', isStaffOrAdmin, validateBody(billingCancelSchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error canceling subscription', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/member-billing/:email/undo-cancellation', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error undoing cancellation', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/member-billing/:email/credit', isStaffOrAdmin, validateBody(billingCreditSchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error applying credit', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/discount', isStaffOrAdmin, validateBody(billingDiscountSchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error applying discount', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.get('/api/member-billing/:email/invoices', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error getting invoices', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.get('/api/member-billing/:email/payment-history', isStaffOrAdmin, validateQuery(paymentHistoryQuerySchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error getting payment history', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/payment-link', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error creating payment link', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/migrate-to-stripe', isStaffOrAdmin, validateBody(billingMigrateSchema), async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error initiating migration', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.post('/api/member-billing/:email/cancel-migration', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error cancelling migration', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
...
router.get('/api/member-billing/:email/migration-status', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[MemberBilling] Error getting migration status', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Billing operation failed', details: safeErrorDetail(error) });
  }
});
