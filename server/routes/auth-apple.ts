...
router.post('/api/auth/apple/link', requireAppleConfig, async (req, res) => {
  try {
    ...
    logMemberAction({
      ...
      req,
    }).catch(err => logger.error('[Apple Auth] Failed to log apple_link action', { extra: { error: getErrorMessage(err) } }));
...
router.post('/api/auth/apple/unlink', requireAppleConfig, async (req, res) => {
  try {
    ...
    logMemberAction({
      ...
      req,
    }).catch(err => logger.error('[Apple Auth] Failed to log apple_unlink action', { extra: { error: getErrorMessage(err) } }));
