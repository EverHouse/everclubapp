...
router.post('/api/stripe/staff/charge-subscription-invoice', isStaffOrAdmin, validateBody(chargeSubscriptionInvoiceSchema), async (req: Request, res: Response) => {
  try {
    ...
    if (paidInvoice.status === 'paid') {
      await db.execute(sql`UPDATE users SET membership_status = 'active', membership_status_changed_at = CASE WHEN membership_status IS DISTINCT FROM 'active' THEN NOW() ELSE membership_status_changed_at END, billing_provider = 'stripe', archived_at = NULL, archived_by = NULL, updated_at = NOW() WHERE id = ${userId}`);
    }
