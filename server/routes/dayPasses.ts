...
router.get('/api/day-passes/products', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[DayPasses] Error getting products', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/day-passes/checkout', checkoutRateLimiter, async (req: Request, res: Response) => {
  try {
    ...
    try { logFromRequest(req, { action: 'create_day_pass', resourceType: 'day_pass', resourceId: session.id, details: { email, productSlug, amountCents: product.priceCents, sessionId: session.id } }); } catch (auditErr) { logger.warn('[Audit] Failed to log create_day_pass:', { extra: { error: getErrorMessage(auditErr) } }); }
    ...
    logger.error('[DayPasses] Error creating checkout session', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/day-passes/confirm', checkoutRateLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!resolvedPaymentIntentId) {
      return res.status(400).json({ error: 'Could not resolve payment intent ID' });
    }
    ...
    try { logFromRequest(req, { action: 'confirm_day_pass_payment', resourceType: 'day_pass', resourceId: String(purchase.id), details: { email, productSlug, amountCents: amountPaid, paymentIntentId: resolvedPaymentIntentId } }); } catch (auditErr) { logger.warn('[Audit] Failed to log confirm_day_pass_payment:', { extra: { error: getErrorMessage(auditErr) } }); }
    ...
    logger.error('[DayPasses] Error confirming payment', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/day-passes/staff-checkout', isStaffOrAdmin, async (req: Request, res: Response) => {
  try {
    ...
    try { logFromRequest(req, { action: 'create_day_pass', resourceType: 'day_pass', resourceId: result.paymentIntentId, details: { email, productSlug, amountCents: product.priceCents, staffEmail, staffInitiated: true, paymentIntentId: result.paymentIntentId } }); } catch (auditErr) { logger.warn('[Audit] Failed to log create_day_pass:', { extra: { error: getErrorMessage(auditErr) } }); }
    ...
    logger.error('[DayPasses] Error creating staff checkout', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/day-passes/staff-checkout/confirm', isStaffOrAdmin, async (req: Request, res: Response) => {
  try {
    ...
    try { logFromRequest(req, { action: 'confirm_day_pass_payment', resourceType: 'day_pass', resourceId: String(purchase.id), details: { email, productSlug, amountCents: paymentIntent.amount, staffEmail, staffInitiated: true, paymentIntentId } }); } catch (auditErr) { logger.warn('[Audit] Failed to log confirm_day_pass_payment:', { extra: { error: getErrorMessage(auditErr) } }); }
    ...
    logger.error('[DayPasses] Error confirming staff checkout', { extra: { error: getErrorMessage(error) } });
    res.status(500).json({ error: 'Failed to confirm payment' });
  }
});
 ...
 */
export { recordDayPassPurchaseFromWebhook } from '../core/billing/dayPassService';
