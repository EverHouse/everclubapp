          ...
          if (!statusMatches) {
            ...
            if (!dryRun) {
              await db.execute(sql`UPDATE users 
                 SET membership_status = ${expectedAppStatus}, membership_status_changed_at = CASE WHEN membership_status IS DISTINCT FROM ${expectedAppStatus} THEN NOW() ELSE membership_status_changed_at END, billing_provider = 'stripe', updated_at = NOW() 
                 WHERE id = ${member.id}`);
...
async function runCleanupInBackground(jobId: string, dryRun: boolean, staffEmail: string, req: Request) {
  ...
  const syncProgress = async () => {
    ...
    await updateJobProgress(jobId, progress as unknown as Record<string, unknown>).catch((err: unknown) => {
      logger.warn('[StripeTools] Failed to update job progress', { extra: { jobId, error: getErrorMessage(err) } });
    });
  };
