  ...
  try {
    ...
    if (sessionId) {
      ...
      if (snapshotCheck.rows.length > 0) {
        ...
        snapshotTotalCents = parseInt((snapshotCheck.rows[0] as DbRow).total_cents as string, 10) || 0;
      }
    }
...
import adminRosterGuestsRouter from './admin-roster-guests';
router.use(adminRosterGuestsRouter);
