...
router.get('/api/admin/trackman/needs-players', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('Error fetching needs-players bookings', { extra: { error: getErrorMessage(error) } });
...
router.delete('/api/admin/trackman/reset-data', isStaffOrAdmin, async (req, res) => {
  ...
  try {
    ...
    logger.error('Trackman reset error', { extra: { error: getErrorMessage(error) } });
...
router.get('/api/admin/backfill-sessions/preview', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[Backfill Preview] Error', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/admin/backfill-sessions', isStaffOrAdmin, async (req, res) => {
  ...
  try {
    ...
    logger.error('[Backfill Sessions] Error', { extra: { error: getErrorMessage(error) } });
...
router.get('/api/admin/trackman/duplicate-bookings', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[Trackman Duplicates] Error', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/admin/trackman/cleanup-duplicates', isStaffOrAdmin, async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    logger.error('[Trackman Cleanup Duplicates] Error', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/admin/repair-linked-email-bookings', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[Admin] Failed to repair linked email bookings', { extra: { error: getErrorMessage(error) } });
