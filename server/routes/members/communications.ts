    ...
    const unifiedVisitsResult = await db.execute(sql`
      SELECT DISTINCT ON (visit_type, visit_id) * FROM (
        ...
        SELECT 
          ...
          'Walk-in' as role,
          (wiv.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')::date::text as date,
          TO_CHAR(wiv.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles', 'HH24:MI:SS') as start_time,
          NULL as end_time,
