    ...
    const fetchBookingRequests = async () => {
      try {
        ...
        return await db.select({
          ...
          resource_type: resources.type,
          declared_player_count: bookingRequests.declaredPlayerCount,
          is_linked_member: sql`LOWER(${bookingRequests.userEmail}) != ${userEmail}`,
          primary_booker_name: bookingRequests.userName,
          invite_status: sql`(SELECT invite_status FROM booking_members WHERE booking_id = ${bookingRequests.id} AND LOWER(user_email) = ${userEmail} LIMIT 1)`,
