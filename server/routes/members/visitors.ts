...
router.get('/api/visitors', isStaffOrAdmin, validateQuery(visitorsQuerySchema), async (req, res) => {
  try {
    ...
    logger.error('Visitors list error', { extra: { error: getErrorMessage(error) } });
...
router.get('/api/visitors/:id/purchases', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('Visitor purchases error', { extra: { error: getErrorMessage(error) } });
...
router.get('/api/guests/needs-email', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[Guests Needs Email] Error', { extra: { error: getErrorMessage(error) } });
  ...
  try {
        ...
        if (createStripeCustomer) {
          if (!stripeCustomerId) {
            try {
              ...
              if (stripeCustomerId) {
                await db.execute(sql`UPDATE users SET stripe_customer_id = ${stripeCustomerId} WHERE id = ${user.id}`);
              }
              logger.info('[Visitors] Stripe customer for linked visitor', { extra: { userId: user.id, stripeCustomerId, isNew: stripeCreated } });
            } catch (stripeErr: unknown) {
              logger.error('[Visitors] Failed to create Stripe customer for linked visitor', { extra: { error: getErrorMessage(stripeErr) } });
            }
          } else {
          ...
          import('../../core/hubspot/members').then(({ findOrCreateHubSpotContact }) => {
            findOrCreateHubSpotContact(
              ...
              undefined,
              { role: 'visitor' }
            ).catch((err: unknown) => logger.error('[Visitors] HubSpot sync failed for linked visitor', { extra: { error: getErrorMessage(err) } }));
          }).catch((err) => { logger.warn('[Visitors] Non-critical HubSpot import failed:', { extra: { error: getErrorMessage(err) } }); });
        }
    ...
    if (createStripeCustomer) {
      try {
        ...
        logger.error('[Visitors] Failed to create Stripe customer for new visitor', { extra: { error: getErrorMessage(stripeErr) } });
      }
      ...
      import('../../core/hubspot/members').then(({ findOrCreateHubSpotContact }) => {
        findOrCreateHubSpotContact(
          ...
          undefined,
          { role: 'visitor' }
        ).catch((err: unknown) => logger.error('[Visitors] HubSpot sync failed for new visitor', { extra: { error: getErrorMessage(err) } }));
      }).catch((err) => { logger.warn('[Visitors] Non-critical HubSpot import failed:', { extra: { error: getErrorMessage(err) } }); });
    }
    ...
    logger.error('[Visitors] Create visitor error', { extra: { error: getErrorMessage(error) } });
...
router.get('/api/visitors/search', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    logger.error('[Visitors] Search error', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/visitors/backfill-types', isAdmin, async (req, res) => {
  try {
    ...
    logger.error('[Visitors] Backfill types error', { extra: { error: getErrorMessage(error) } });
...
router.delete('/api/visitors/:id', isStaffOrAdmin, async (req, res) => {
  try {
    ...
    await db.execute(sql`DELETE FROM account_deletion_requests WHERE LOWER(email) = ${visitorEmail}`);
    ...
    if (deleteFromStripe === 'true' && visitor.stripeCustomerId) {
      try {
        ...
        logger.error('[Visitors] Failed to delete Stripe customer', { extra: { stripeCustomerId: visitor.stripeCustomerId, error: getErrorMessage(stripeError) } });
      }
    }
    ...
    logger.info('[Visitors] Visitor permanently deleted: () by . Records', { extra: { visitorEmail: visitor.email, visitorName, sessionUser: sessionUser?.email, deletionLogJoin: deletionLog.join(', ') } });
    ...
    logger.error('[Visitors] Delete error', { extra: { error: getErrorMessage(error) } });
...
router.get('/api/visitors/check-email', isStaffOrAdmin, validateQuery(checkEmailSchema), async (req, res) => {
  try {
    ...
    logger.error('[Visitors] Check email error', { extra: { error: getErrorMessage(error) } });
