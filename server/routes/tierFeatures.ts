...
router.get('/api/tier-features', async (req, res) => {
  try {
        ...
        if (existingValue && existingValue.value !== null && existingValue.value !== '' && existingValue.value !== false) {
          ...
          if (mapping.column === null) {
            ...
            if (mapping.format) {
              ...
              derivedValue = rawValue as string;
            }
          }
    ...
    logger.error('Tier features fetch error', { extra: { error: getErrorMessage(error) } });
...
router.post('/api/tier-features', isAdmin, async (req, res) => {
  ...
  try {
    ...
    logger.error('Create tier feature error', { extra: { error: getErrorMessage(error) } });
...
router.put('/api/tier-features/:id', isAdmin, async (req, res) => {
  try {
    ...
    logger.error('Update tier feature error', { extra: { error: getErrorMessage(error) } });
...
router.delete('/api/tier-features/:id', isAdmin, async (req, res) => {
  try {
    ...
    logger.error('Delete tier feature error', { extra: { error: getErrorMessage(error) } });
...
router.put('/api/tier-features/:featureId/values/:tierId', isAdmin, async (req, res) => {
  try {
    ...
    logger.error('Update tier feature value error', { extra: { error: getErrorMessage(error) } });
