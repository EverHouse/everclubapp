...
export const TRAINING_SEED_DATA = [
  {
    ...
    steps: [
      ...
      { title: 'Sidebar Navigation', content: 'On larger screens (desktop), the sidebar provides quick access to all main sections plus admin tools. The sidebar mirrors the hamburger menu items.'},
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Payment Methods', content: 'Four payment options appear in the Booking Sheet: "Card Reader" (tap/insert on the Stripe Terminal — this is the default), "Charge Card on File" (use the member\'s saved card, showing brand and last 4 digits), "Pay with Card" (enter a new card online), or "Waive All Fees" (forgive fees with a required reason).' },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Search', content: 'Use the search bar to find people by name, email, phone, tier, or discount code.' },
      ...
      { title: 'Member Profile Drawer', content: 'Tap a member to open their profile drawer with tabs for: Overview (contact info, tier, discount code), Billing (subscription, payments, group membership), Activity (bookings, events, visit history), Notes (staff notes and pinned notes), and Communications (email and SMS logs).' },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Status Pipeline', content: 'Applications move through a pipeline: New (just submitted), Reviewing (staff is evaluating), Approved (accepted), Invited (checkout link sent), Converted (became a member), or Declined (rejected). Use the status tabs at the top to filter by stage.' },
      { title: 'Viewing an Application', content: 'Tap any application to open its details. You\'ll see the applicant\'s name, email, phone, and any message they included. New applications are automatically marked as "read" when you open them.' },
      ...
      { title: 'Updating Status', content: 'Use the status buttons in the application detail to move it through the pipeline. For example, move from "New" to "Reviewing" when you start evaluating, then to "Approved" when accepted.' },
      { title: 'Sending a Checkout Invite', content: 'For approved applications, select the membership tier you want to offer, then tap "Send Checkout Invite." The applicant receives an email with a link to complete their membership signup and payment.' },
      { title: 'Converted Members', content: 'Once an applicant completes their checkout and becomes a member, their status automatically changes to "Converted." You can see their membership status and tier in the application detail.' },
