...
import { sendPushNotificationToStaff } from '../routes/push';
import { checkDailyBookingLimit, getMemberTierByEmail, getTierLimits, getDailyBookedMinutes } from '../core/tierService';
import { notifyAllStaff } from '../core/notificationService';
import { formatDateDisplayWithDay, formatTime12Hour, createPacificDate } from '../utils/dateUtils';
...
import { getCalendarNameForBayAsync, isStaffOrAdminCheck } from '../routes/bays/helpers';
...
export async function listBookingRequests(req: Request, res: Response) {
  try {
    ...
    if (isStaffRequest) {
      ...
      if (!hasStaffAccess) {
        return res.status(HTTP_STATUS.FORBIDDEN).json({ error: 'Staff access required to view all requests' });
      }
    } else if (user_email) {
      if (requestedEmail !== sessionEmail) {
        const hasStaffAccess = await isStaffOrAdminCheck(sessionEmail);
        if (!hasStaffAccess) {
          return res.status(HTTP_STATUS.FORBIDDEN).json({ error: 'You can only view your own booking requests' });
        }
      }
    } else {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'user_email or include_all parameter required' });
    }
    ...
    const limit = limitParam ? Math.min(parseInt(limitParam as string), MAX_QUERY_LIMIT) : undefined;
    ...
    const pendingBookings = result.filter(b => b.status === BOOKING_STATUS.PENDING || b.status === BOOKING_STATUS.PENDING_APPROVAL);
    ...
    if (pendingBookings.length > 0) {
      ...
      if (pendingPairs.length > 0) {
        ...
        .where(and(
          ...
          or(
            eq(bookingRequests.status, BOOKING_STATUS.APPROVED),
            eq(bookingRequests.status, BOOKING_STATUS.CONFIRMED),
            eq(bookingRequests.status, BOOKING_STATUS.ATTENDED)
          )
        ));
    ...
    logAndRespond(req, res, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'Failed to fetch booking requests', error);
  }
}
...
export async function createBookingRequest(req: Request, res: Response) {
  try {
    ...
    if (isNaN(parsedDate.getTime())) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Invalid date format' });
    }
    ...
    if (validatedDate.getFullYear() !== year || 
      ...
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Invalid date - date does not exist (e.g., Feb 30)' });
    }
    ...
    if (sessionEmail !== requestEmail) {
      const hasStaffAccess = await isStaffOrAdminCheck(sessionEmail);
      if (!hasStaffAccess) {
        return res.status(HTTP_STATUS.FORBIDDEN).json({ error: 'You can only create booking requests for yourself' });
      }
    }
    ...
    if (typeof duration_minutes !== 'number' || !Number.isInteger(duration_minutes) || duration_minutes <= 0 || duration_minutes > MAX_DURATION_MINUTES) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: `Invalid duration. Must be a whole number between 1 and ${MAX_DURATION_MINUTES} minutes.` });
    }
    ...
    if (endHours >= MIDNIGHT_HOUR) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Booking cannot extend past midnight. Please choose an earlier start time or shorter duration.' });
    }
    ...
    try {
      ...
      if (resource_id) {
        ...
        if (overlapCheck.rows.length > 0) {
          await client.query('ROLLBACK');
          return res.status(HTTP_STATUS.CONFLICT).json({ error: 'This time slot is already booked' });
        }
      }
      ...
      if (request_participants && Array.isArray(request_participants)) {
        sanitizedParticipants = request_participants
          .slice(0, MAX_PARTICIPANTS_PER_BOOKING)
          .map((p: any) => ({
        ...
        if (tierName) {
          ...
          if (totalCents > 0) {
            ...
            if (prepaymentCheck.rows.length === 0) {
              await client.query('ROLLBACK');
              return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Prepayment not found' });
            }
    ...
    try {
      ...
      bookingEvents.publish('booking_created', {
        ...
        status: row.status || BOOKING_STATUS.PENDING,
        actionBy: 'member'
    ...
    if (constraint.type === 'unique') {
      return res.status(HTTP_STATUS.CONFLICT).json({ error: 'This time slot may have just been booked. Please refresh and try again.' });
    }
    if (constraint.type === 'foreign_key') {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Referenced record not found. Please refresh and try again.' });
    }
    logAndRespond(req, res, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'Failed to create booking request', error);
  }
}
...
export async function getBookingRequestById(req: Request, res: Response) {
  try {
    ...
    if (isNaN(bookingId)) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Invalid booking ID' });
    }
    ...
    logAndRespond(req, res, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'Failed to fetch booking request', error);
  }
}

export async function memberCancelBooking(req: Request, res: Response) {
  try {
    const { id } = req.params;
    ...
    if (!isOwnBooking && !isValidViewAs && !isLinkedEmail) {
      ...
      return res.status(HTTP_STATUS.FORBIDDEN).json({ error: 'You can only cancel your own bookings' });
    }
    
    if (existing.status === BOOKING_STATUS.CANCELLED || existing.status === BOOKING_STATUS.DECLINED) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Booking is already cancelled' });
    }
    
    if (existing.status === BOOKING_STATUS.CANCELLATION_PENDING) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Cancellation is already in progress' });
    }
    
    const wasApproved = existing.status === BOOKING_STATUS.APPROVED;
    ...
    const shouldSkipRefund = hoursUntilStart < LATE_CANCEL_HOURS;
    ...
    logAndRespond(req, res, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'Failed to cancel booking', error);
  }
}
...
export async function calculateFeeEstimate(params: {
  ...
  const isUnlimitedTier = dailyAllowance >= UNLIMITED_TIER_THRESHOLD;
...
export async function getFeeEstimate(req: Request, res: Response) {
  ...
  try {
    ...
    if (bookingId) {
      if (!isStaff) {
        return res.status(HTTP_STATUS.FORBIDDEN).json({ error: 'Staff access required' });
      }
      
      const booking = await db.select().from(bookingRequests).where(eq(bookingRequests.id, bookingId)).limit(1);
      if (!booking.length) {
        return res.status(HTTP_STATUS.NOT_FOUND).json({ error: 'Booking request not found' });
      }
    ...
    logAndRespond(req, res, HTTP_STATUS.INTERNAL_SERVER_ERROR, 'Failed to calculate fee estimate', error);
  }
}

// Staff-only endpoint to get fee estimate for existing booking request
export async function getBookingFeeEstimate(req: Request, res: Response) {
  res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
  res.setHeader('Pragma', 'no-cache');
  res.setHeader('Expires', '0');
