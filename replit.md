...
- **Database & Data Integrity**: Uses PostgreSQL, Supabase Realtime, and Drizzle ORM with CASCADE constraints. Supports Supabase PgBouncer connection pooling via `DATABASE_POOLER_URL` env var (when set, Drizzle uses `prepare: false`; WebSocket pool always uses direct `DATABASE_URL`).
