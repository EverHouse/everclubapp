...
- **Resource Operating Hours**: Per-resource-type (golf, conference, wellness, tours) start/end hours and slot durations configurable from settings. `getResourceConfig()` async function reads from settings with `CALENDAR_CONFIG` defaults.
