...
## Recent Changes
- **Feb 21, 2026**: Ghost column fix (v7.95.0) — Fixed 6 pre-existing SQL bugs where queries referenced non-existent columns (`booking_id`, `booking_request_id`) on `booking_sessions` and `booking_participants` tables. Fixed: member cancel session lookup, Stripe payment participant query, webhook refund audit log, Trackman admin lesson cleanup (2 queries), Trackman import post-import waiver query. All now use proper `trackman_booking_id` JOINs through `booking_requests`. Added CI guard script (`scripts/check-ghost-columns.sh`) that runs automatically before every build to prevent regressions.
- **Feb 21, 2026**: Legacy table migration complete (v7.94.0) — Dropped `booking_members` and `booking_guests` tables from the database. Migrated all 20+ read queries across backend to use `booking_participants` exclusively. Removed Drizzle schema definitions, type exports, dead `populateBookingMembers` function, and Trackman backfill logic. `booking_participants` is now the single source of truth for all roster management — no legacy tables remain.
