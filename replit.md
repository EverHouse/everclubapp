...
### Recent Changes
- **Email Templates: Full Coverage & Non-Toggleable Categories (v8.80.0)**: Extracted inline OTP login email from `auth.ts` → `server/emails/otpEmail.ts` and tour confirmation email from `tours.ts` → `server/emails/tourEmails.ts`. Both registered in `emailTemplatePreview.ts` with preview data. Admin Email Templates tab now shows Authentication category (first position, lock icon) with "Always On" badge and non-toggleable UI — system-critical emails cannot be disabled. Tour Confirmation added under Booking category. Template count: 19 (was 17).
