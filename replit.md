...
- **Type Safety**: Fix underlying schemas or DTOs to resolve TypeScript mismatches; `as any` and `Record<string, any>` are forbidden (codebase has 0 of each). All raw SQL `db.execute()` results must be typed with row interfaces and cast via `as unknown as RowType[]`. Stripe SDK expanded properties use typed interfaces (e.g., `StripeInvoiceExpanded`, `StripeSubscriptionExpanded`) instead of generic Record casts. HubSpot responses use typed contact/result interfaces. The codebase maintains 0 TypeScript errors. Only 3 intentional `: any` remain (logger index signature, 2 Drizzle auto-seed functions with eslint-disable).
