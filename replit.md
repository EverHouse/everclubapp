...
## Recent Changes
- **Feb 21, 2026**: Legacy system cleanup (v7.92.0) — Deleted dead `inviteExpiryScheduler`. Removed all writes to `booking_members` (10+ files) and `booking_guests` (7 files) — `booking_participants` is now the sole roster source of truth. Changed `invite_status` DB default from 'pending' to 'accepted' and removed redundant hardcoded values from all participant inserts. Tables and read queries preserved temporarily for data migration safety. Legacy systems deferred: overage.ts route (still used by check-in billing), legacyPurchases.ts (still used by admin tools), MindBody billing_provider guards (still needed for billing authority checks).
