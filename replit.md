...
- **Resource Operating Hours**: Single club-wide open/close hours (`resource.club_open_hour`, `resource.club_close_hour`) apply to all bookable resources. Per-resource slot durations (golf, conference, tours) are individually configurable. Wellness & Classes have no configurable hours (they are Google Calendar events). `getResourceConfig()` async function reads from settings with `CALENDAR_CONFIG` defaults.
