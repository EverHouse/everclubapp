...
**Current Version**: 8.73.0 (March 5, 2026)
...
- **Settings Infrastructure**: Key-value store in `system_settings` table, cached via `settingsHelper.ts` (30s TTL), bulk save via `PUT /api/admin/settings`. Public settings exposed via unauthenticated `GET /api/settings/public` (contact, social, apple_messages, hours_display categories only).
...
- **Resource Operating Hours**: Availability hours are derived from the Display Hours settings (`hours.monday`, `hours.tuesday_thursday`, `hours.friday_saturday`, `hours.sunday`) — parsed per day of week. Monday "Closed" = no bookable slots. "8:30 AM – 8:00 PM" = slots from 9 AM to 8 PM. Per-resource slot durations (golf=60, conference=30, tours=30) are individually configurable. Wellness & Classes have no configurable hours (Google Calendar events). `getResourceConfig(resourceType, date?)` reads display hours for the given date with `CALENDAR_CONFIG` as fallback when no date is provided.
